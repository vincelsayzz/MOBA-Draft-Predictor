...
function App() {
  ...
  // --- UPDATED: 6 CORE ITEMS + 1 NEUTRAL GENERATOR ---
  // Uses strict internal API names to prevent blank images (e.g. greater_crit = Daedalus, ultimate_scepter = Aghs)
  ...
  const handleMouseMove = (e) => {
    let x = e.clientX + 15;
    let y = e.clientY + 15;
    // Increased the offset check slightly because the modal is wider now
  ...
  const handleHeroPick = (hero) => {
    ...
    if (pickingFor === 'Radiant') {
      if (radiantHeroes.length < 5) setRadiantHeroes([...radiantHeroes, hero]);
      else alert("Radiant already has 5 heroes!");
    } else {
      if (direHeroes.length < 5) setDireHeroes([...direHeroes, hero]);
      else alert("Dire already has 5 heroes!");
    }
    setSearchQuery('');
    setHoveredHero(null); 
  };
  ...
  const handlePredict = async () => {
    if (radiantHeroes.length !== 5 || direHeroes.length !== 5) {
      alert("You must draft exactly 5 heroes per team to predict!");
      return;
    }
    setLoading(true);
    setResults(null);
    ...
    <>
      ...
      {hoveredHero && heroApiData[hoveredHero.id] && (
        <div style={{
          ...
          <div style={styles.tooltipBody}>
            ...
            {/* NEW: Live Win Rate Progress Bar */}
            ...
            {/* NEW: 6 Core Items + 1 Neutral Item */}
      ...
      <div style={styles.container}>
        <header style={styles.header}>
          <h1 style={styles.appTitle}>DOTA 2 <span style={{color: '#66fcf1'}}>DRAFT SIMULATOR</span></h1>
...
const styles = {
  ...
  // NEW: Updated Hover Tooltip Styles
  hoverTooltip: {
